...
uint64_t measureRecvBytes = 0;
uint64_t measureSendBytes = 0;
uint64_t measureDropBytes = 0;

void measureSendedBytes(uint32_t n){
    measureSendBytes += n;
}

void measureRecvedBytes(uint32_t n){
    measureRecvBytes += n;
}

void measureDropedBytes(uint32_t n){
    measureDropBytes += n;
}
...
void measureDumpToLog()
{
	...
	for(iter = uriTimeCount.begin(); iter != uriTimeCount.end(); iter++){
		...
		if(costTime > 0){
			...
			log(Notice, "measure: %4u|%3u = %8llu, %7u, %7llu, %7llu", uri_h, uri_t, 
...
std::string measureDumpJSON(bool getFrom, bool getFromIp, bool getAndClear)
{
    ...
    // send/recv band 
    {
		os << "		\"band\":" << std::endl;
		os << "		{" << std::endl;
		os << "			\"send\":" << int2str(measureSendBytes) << "," << std::endl;
		os << "			\"recv\":" << int2str(measureRecvBytes) << "," << std::endl;
		os << "			\"drop\":" << int2str(measureDropBytes) << std::endl;
		os << "		},\n";
    }
